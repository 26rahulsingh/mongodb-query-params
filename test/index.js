...
const requiredmqp = require('../src');

test('module imported using require', t => {
  const res = requiredmqp('key=value');
  t.truthy(res);
});

test('filter: basic', t => {
  const res = mqp('key=value');
  t.truthy(res);
  t.deepEqual(res.filter, { key: 'value' });
});

test('filter: number casting', t => {
  const res = mqp('key1=10&key2=1.2&key3=0&key4=0001');
  t.truthy(res);
...
test('filter: boolean casting', t => {
  const res = mqp('key1=true&key2=false');
  t.truthy(res);
  t.deepEqual(res.filter, { key1: true, key2: false });
});

test('filter: regex casting', t => {
  const res = mqp('key1=/regex/&key2=/regexi/i&key3=/^regex, with comma$/');
  t.truthy(res);
  t.deepEqual(res.filter, {
    key1: /regex/,
    key2: /regexi/i,
...
test('filter: date casting', t => {
  const res = mqp('key1=2016-04&key2=2016-04-12&key3=2016-04-02 08:00');
  t.truthy(res);
  t.deepEqual(res.filter, {
...
test('filter: ignore default keys', t => {
  const res = mqp('key=value&skip=0&limit=10&fields=id,name&sort=name');
  t.truthy(res);
  t.truthy(res.filter);
...
test('filter: ignore blacklisted keys', t => {
  const res = mqp('key1>value1&key2=value2&key3<=value3', {
    blacklist: ['key1', 'key3'],
  });
  t.truthy(res);
  t.truthy(res.filter);
...
test('filter: ignore all but whitelisted keys', t => {
  const res = mqp('key1>value1&key2=value2&!key3', {
    whitelist: ['key2'],
  });
  t.truthy(res);
  t.truthy(res.filter);
...
test('skip', t => {
  const res = mqp('skip=10');
  t.truthy(res);
  t.is(res.skip, 10);
});

test('skip (custom key)', t => {
  const res = mqp('offset=10', { skipKey: 'offset' });
  t.truthy(res);
  t.is(res.skip, 10);
});

test('limit', t => {
  const res = mqp('limit=10');
  t.truthy(res);
  t.is(res.limit, 10);
});

test('limit (custom key)', t => {
  const res = mqp('max=10', { limitKey: 'max' });
  t.truthy(res);
  t.is(res.limit, 10);
});

test('projection (includes)', t => {
  const res = mqp('fields=a,b,c');
  t.truthy(res);
  t.deepEqual(res.projection, { a: 1, b: 1, c: 1 });
});

test('projection (includes + _id exclude)', t => {
  const res = mqp('fields=a,b,-_id');
  t.truthy(res);
...
test('projection (multiple keys)', t => {
  const res = mqp('fields=a&fields=b');
  t.truthy(res);
  t.deepEqual(res.projection, { a: 1, b: 1 });
});

test('projection (custom key)', t => {
  const res = mqp('select=a,b,c', { projectionKey: 'select' });
  t.truthy(res);
  t.deepEqual(res.projection, { a: 1, b: 1, c: 1 });
});

test('sort', t => {
  const res = mqp('sort=a,+b,-c');
  t.truthy(res);
  t.deepEqual(res.sort, { a: 1, b: 1, c: -1 });
});

test('sort (multiple keys)', t => {
  const res = mqp('sort=a&sort=-b');
  t.truthy(res);
  t.deepEqual(res.sort, { a: 1, b: -1 });
});

test('sort (custom key)', t => {
  const res = mqp('order=a,-b', { sortKey: 'order' });
  t.truthy(res);
  t.deepEqual(res.sort, { a: 1, b: -1 });
});

test('complex response', t => {
  const res = mqp(`sort=+a,-b&skip=10&limit=50
...
test('query already parsed', t => {
  const res = mqp({ key: 'foo', limit: '50' });
  t.deepEqual(res, {
    filter: {
      key: 'foo',
    },
